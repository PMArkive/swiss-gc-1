...
void GuiImage::Draw()
{
	if((!image && !imageData) || !this->IsVisible() || tile == 0) {
		return;
	}
	...
	{
		if(imageData) {
			Menu_DrawImg(currLeft, thisTop, imageData, imageangle, currScaleX, currScaleY, this->GetAlpha());
		}
		else {
...
{
	return imageData ? imageData->GetWidth() : this->width;
}

int GuiImage::GetHeight()
{
	return imageData ? imageData->GetHeight() : this->height;
}
