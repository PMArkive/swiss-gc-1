...
void Menu_DrawImg(f32 xpos, f32 ypos, GuiImageData *image, f32 degrees, f32 scaleX, f32 scaleY, u8 alpha)
{
	if(image == NULL)
		return;

	if(image->GetTex() == NULL) {
		Menu_DrawImg(xpos, ypos, image->GetWidth(), image->GetHeight(), image->GetImage(), degrees, scaleX, scaleY, alpha);
		return;
	}
	GXTexObj * texObj = image->GetTex();
